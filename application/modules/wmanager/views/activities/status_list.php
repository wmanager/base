...
if (isset ( $status_data )) {
	foreach ( $status_data as $key => $status_item ) {
		?>
		<tr>
			<td><input type="text" value="<?=$status_item->key;?>"
				class="form-control" name="status_key[]" aria-required="true"
				required="required"></td>
			<td><input type="text" value="<?=$status_item->label;?>"
				class="form-control" name="label[]"></td>
			<td><input type="text" value="<?=$status_item->description;?>"
				class="form-control" name="status_description[]"></td>
			<td><input type="number" value="<?=$status_item->ordering;?>" min="0"
				name="ordering[]" class="form-control"></td>
			...
			<td><a
				href="<?=site_url('/admin/setup_activities/delete_status/'.$id_process.'/'.$activity_id.'/'.$status_item->id);?>"
				class="delete-confirm"
				data-message="Are you sure.Do you want to delete the record?"
				title="Delete"><i class="fa fa-trash-o"></i></a></td>
			<td><a
				href='<?=site_url('/admin/setup_activities/add_transition/'.$id_process.'/'.$activity_id.'/'.$status_item->key);?>'
				data-target='#transition-modal' data-toggle='modal'
				title="Transition"><i
					class="fa fa-exchange <?php if($status_item->transition_count > 0) echo "link_color_green";?>"
					aria-hidden="true"></i></a></td>
			<input type="hidden" name="val_id[]" value="<?=$status_item->id;?>" />
		</tr>
<?php
	}
}
