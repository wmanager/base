...
class Excel_XML
{
	 ...
	 */
        ...
        */
        ...
        {    	
                ...
                foreach ($array as $k => $v):
                        ...
                        if(trim($v) != ''){
                            ...
                            if($type == 'DateTime') {
                                $v = date('Y-m-d',strtotime($v)).'T'.date('H:i:s.u',strtotime($v));
        ...
        public function saveXML ($filename = 'excel-export')
        {
        	// correct/validate filename
        	$filename = preg_replace('/[^aA-zZ0-9\_\-]/', '', $filename);
        	...
        	// print out document to the browser
        	// need to use stripslashes for the damn ">"
        	$str_data = stripslashes (sprintf($this->header, $this->sEncoding));
        	$str_data .= "\n<Worksheet ss:Name=\"" . $this->sWorksheetTitle . "\">\n<Table>\n";
        	foreach ($this->lines as $line)
        		$str_data .= $line;
        		//$str_data .= "</Table>\n</Worksheet>\n<AutoFilter x:Range=\"R4C1:R4C6\" xmlns=\"urn:schemas-microsoft-com:office:excel\> </AutoFilter>\n";
        		$str_data .= "</Table>\n</Worksheet>\n";
        		$str_data .= $this->footer;
        		return $str_data;
        }
}
