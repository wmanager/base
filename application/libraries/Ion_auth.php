	...
	public function forgotten_password($identity) // changed $email to $identity
{
			...
			if ($user) {
				$data = array (
					...
					$data['user'] = $user;
					$extra_mapping['template_values'] = $data;
					$queue_details = array();
					$extra_mapping['queue_values'] = $queue_details;
					$attachment_array = array();
					$this->load->library('email_notification_library');
					if ($this->email_notification_library->send_email('FORGOT_PASSWORD', $user->email, NULL, NULL, $extra_mapping, $attachment_array)) {
	 ...
	 *        
	 */
	public function register($username, $password, $email, $additional_data = array(), $roles_ids = array(), $group_ids = array()) // need to test email activation
{
		...
		if (! $email_activation) {
			$id = $this->ion_auth_model->register ( $username, $password, $email, $additional_data, $roles_ids, $group_ids );
			if ($id !== FALSE) {
