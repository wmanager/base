...
class Form_builder {
	...
	/**
	 *
	 * @property array $input_addons This is for adding input-groups and addons.
	 ...
	 *
	 * @param array $ary
	 *        	- an array from the DB. Format: $k => $v
	 * @param array $custom_options
	 *        	- optional, an array that will override
	...
	/**
	 *
	 * @param array $pre_built
	 *        	- the array that was pre-built using `auto_db_to_options`
	 * @param string $id
	 *        	- the id/name of the element to add to
	 * @param array $vals_ary
	 *        	- an array of new (or over-riding) values
	 * @return none @ussage
	 ...
	 * @access public
	 * @param
	 *        	Array - The array of options for the form.
	 *        	array(
	 *        	array(
			...
			if (is_array ( $this->elm_options )) {
				...
				switch ($this->func) {
					...
					case 'form_checkbox' :
						...
						$this->print_string .= '<div class="col-md-2"></div>';
						$this->print_string .= $this->_build_input ();
						$this->print_string .= $this->_post_elm ();
						break;
					default :
						if (isset ( $this->elm_options ['column'] )) {
							$this->print_string .= '<div class="' . $this->elm_options ['column'] . '">';
							$this->print_string .= $this->_pre_elm ();
	 ...
	 *
	 * @param array $options
	 *        	- the elements/options for the form elemnts being built.
	 * @param array/object $data_source
	 *        	- a default source for the value
	...
	function build_display($options, $data_source = array()) {
		...
		$this->print_string .= $this->_build_form_open ( '', array (
				'class' => $this->config ['default_form_class'] 
		) );
	...
	private function _build_input($include_pre_post = true) {
		...
		if ($this->func == 'form_combine') {
			...
			if (isset ( $elm_options_backup ['elements'] [0] ['column'] )) {
			...
			foreach ( $elm_options_backup ['elements'] as $elm ) {
				...
				if (isset ( $this->elm_options ['column'] )) {
					$input_html_string .= '<div class="' . $this->elm_options ['column'] . '">';
				}
				$input_html_string .= $this->_build_input ( true );
				if (isset ( $this->elm_options ['column'] )) {
			...
			switch ($this->func) {
				...
				case 'form_checkbox' :
					...
					unset ( $this->elm_options ['label'] );
					$this->elm_options ['class'] = 'checkbox';
					if ($this->elm_options ['value'] && ($this->elm_options ['value'] == 't' || $this->elm_options ['value'] == 'true')) {
						$this->elm_options ['checked'] = TRUE;
					} else {
						$this->elm_options ['value'] = $this->elm_options ['default_value'];
					}
					// print_r($this->elm_options);
					$input_html_string = '<div class="checkbox"><label>' . form_checkbox ( $this->elm_options ) . ' ' . $label . '</label></div>';
					break;
				case 'form_radio' :
					$label = $this->elm_options ['label'];
					unset ( $this->elm_options ['label'] );
					...
					$input_html_string = '<div class="radio"><label>' . form_radio ( $this->elm_options ) . ' ' . $label . '</label></div>';
					break;
				case 'form_date' :
					$this->input_addons ['exists'] = true;
