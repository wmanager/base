...
class Ion_auth_model extends CI_Model {
	 ...
	 *        
	 */
	public function register($username, $password, $email, $additional_data = array(), $roles = array(), $groups = array()) {
	...
	public function login($identity, $password, $remember = FALSE) {
		...
		$query = $this->db->select ( $this->tables ['users'] . '.' . $this->identity_column . ', ' . $this->tables ['users'] . '.username, ' . $this->tables ['users'] . '.email, ' . $this->tables ['users'] . '.id, ' . $this->tables ['users'] . '.password, ' . $this->tables ['users'] . '.active, ' . $this->tables ['users'] . '.last_login' )->where ( $this->tables ['users'] . '.' . $this->identity_column, $this->db->escape_str ( $identity ) )->join ( 'companies', 'companies.id = ' . $this->tables ['users'] . '.id_company' )->where ( 'companies.active', 't' )->limit ( 1 )->get ( $this->tables ['users'] );
	...
	public function add_to_role($role_id, $user_id = false) {
		$data = array (
				'role_id' => $role_id,
				'user_id' => $user_id 
		);
		$this->db->insert ( 'setup_users_roles', $data );
		$this->set_message ( 'Role creation successful' );
		return true;
	}
